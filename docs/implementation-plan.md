...
- Responsive Topbar overlap fix: `pnpm --filter @silksong-git/web test` passed with 50 tests, the Web production build passed, and Chromium DOMRect probes of Static loaded mode found no top-of-viewport overflow and kept the main content bottom aligned with the viewport; at 1600px ModeBanner and the controls shared one row, while at 1480, 1479, 1475, 1400, 1266, 1234, 1000, and 800px ModeBanner occupied the first row and the complete controls group started below it
