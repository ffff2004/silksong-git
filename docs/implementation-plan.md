...
- Responsive Topbar overlap fix: `pnpm --filter @silksong-git/web test` passed with 50 jsdom tests and 1 Chromium Browser test, the Web production build passed, and Chromium DOMRect checks found no top-of-viewport overflow and kept the main content bottom aligned with the viewport; the dedicated regression test verifies that ModeBanner and the controls share a row at 1600px and that the complete controls group starts below ModeBanner at 1234px, while a deliberate `nowrap` probe failed at that seam and produced Git-ignored failure screenshots and attachments under `apps/web/test-results/`
...
- Fixed responsive Topbar overlap by keeping `ModeBanner` in a wrapping primary-row flex flow. The complete controls group uses its intrinsic width for outer line allocation, so insufficient space moves it below ModeBanner before its own controls wrap; `AppShell` allocates the remaining viewport height to its existing main scroll container. A dedicated Vitest Browser regression test renders the complete Static Web Mode application in Chromium and checks the semantic Topbar layout seam at wide and constrained viewports.
...
- [x] Step 4: migrate the Map cluster in dependency order.
  - [x] Migrate `InteractiveMapCanvas`, including page/modal variants, stage, image, pins, markers, and obtained state; expose stable `data-variant` or semantic DOM state for tests.
