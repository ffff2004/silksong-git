...
The Local History API Process is a singleton per Save History Repository. `watch start` should hold a long-lived repository-scoped watch lock, separate from the short-lived write lock used for Git and SQLite mutations, so a second watcher cannot start for the same repository while Offline Commands can still serialize legitimate writes.
