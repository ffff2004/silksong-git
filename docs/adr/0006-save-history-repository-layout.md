...
We decided that a Save History Repository worktree stores the current commit's `save.dat`, `decoded-save.json`, and `observation.json`, plus `.silksong-git/config.json` for Project Config. `observation.json` includes raw-observation metadata and Version Stamps such as `decoderVersion`. The SQLite Semantic Read Model lives at `.silksong-git/read-model.sqlite` by default but is ignored by Git, because it can be rebuilt from the raw observation commits. The runtime write lock lives at `.silksong-git/write.lock` and is also ignored by Git.
