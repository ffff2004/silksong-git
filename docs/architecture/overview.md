...
Desktop Rust -------JSONL------> Desktop sidecar ---> @silksong-git/repo-session
                                      +------------> @silksong-git/history (managed lifecycle seams)
...
| [`packages/history`](../../packages/history/src/index.ts)           | Owns one Save History Repository, Git and SQLite adapters, observation and restore workflows, query/diff/search behavior, repository locks, watcher leases, generic repository relocation, and reversible replacement transactions. It depends on Core for semantic interpretation.                                                                                                                                                                                                                                                                         |
...
| [`apps/desktop-sidecar`](../../apps/desktop-sidecar/src/main.ts)    | Provides the private versioned JSONL process Adapter for Repo Session lifecycle commands, managed initialization, Desktop archive intents mapped to generic relocation, Watched Save comparison, confirmed migration, the prepare/resolve replacement workflow, and stateless Core save inspection through public package Interfaces. Desktop Rust communicates with the Cargo-selected, preflight-validated Runtime Layout over its stdin/stdout protocol. See the [Desktop Sidecar Process Protocol Reference](../reference/desktop-sidecar-protocol.md). |
...
The Desktop replacement workflow validates the selected Encoded Save and finds
the eligible Managed Repository whose canonical Watched Save identity matches
it. After Desktop-owned session cleanup, Desktop computes the archive target path
and the sidecar asks History to prepare a generic reversible relocation while
History holds its writer and watcher exclusion.
Desktop then initializes the exact claimed replacement directory, commits its
baseline through the normal public initialization workflow, opens a new Repo
Session, and starts watching before resolving the History operation as committed.
Any replacement failure resolves the same History operation as a rollback and
reports the managed, archive, and replacement-residual paths when recovery is
not complete. The old repository remains an Archived Repository after success.
...
- Managed replacement is an explicit direct-History exception: History owns the
  generic reversible relocation, leases, and rollback, while Desktop owns selected
  save validation, source identity discovery, exact replacement-directory
  cleanup, session transition ordering, and the final commit/rollback result.
- Standalone Desktop archive is a narrow direct-History exception that accepts
  uninspectable direct managed children. History owns generic relocation,
  continuous writer and watcher exclusion, and atomic rename; Desktop owns placement, naming,
  confirmation, and Repo Session transition ordering.
