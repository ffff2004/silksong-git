...
| [`packages/history`](../../packages/history/src/index.ts)           | Owns one Save History Repository, Git and SQLite adapters, observation and restore workflows, query/diff/search behavior, repository locks, watcher leases, and the reversible managed-replacement archive transaction. It depends on Core for semantic interpretation.                                                                                                                                                                                                                                                                 |
...
| [`apps/desktop-sidecar`](../../apps/desktop-sidecar/src/main.ts)    | Provides the private versioned JSONL process Adapter for Repo Session lifecycle commands, managed initialization, standalone managed archive moves, Watched Save comparison, confirmed migration, the prepare/resolve managed-replacement workflow, and stateless Core save inspection through public package Interfaces. Desktop Rust spawns its fixed development entry through Node and communicates over its stdin/stdout protocol. See the [Desktop Sidecar Process Protocol Reference](../reference/desktop-sidecar-protocol.md). |
...
it. After Desktop-owned session cleanup, the sidecar asks History to prepare a
reversible archive move while History holds its writer and watcher exclusion.
...
- Managed replacement is an explicit direct-History exception: History owns the
  reversible archive move, leases, and rollback, while Desktop owns selected
...
- Standalone Desktop archive is a narrow direct-History exception that accepts
  uninspectable direct managed children. History owns continuous writer and
  watcher exclusion plus atomic rename; Desktop owns placement, naming,
