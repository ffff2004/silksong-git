...
| [`apps/desktop-sidecar`](../../apps/desktop-sidecar/src/main.ts)    | Provides the private versioned JSONL process Adapter for Repo Session lifecycle commands, managed initialization, Desktop archive intents mapped to generic relocation, Watched Save comparison, confirmed migration, the prepare/resolve replacement workflow, and stateless Core save inspection through public package Interfaces. Desktop Rust spawns its fixed development entry through Node and communicates over its stdin/stdout protocol. See the [Desktop Sidecar Process Protocol Reference](../reference/desktop-sidecar-protocol.md). |
